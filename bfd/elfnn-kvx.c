...
static char *
kvx_write_core_note (bfd *abfd ATTRIBUTE_UNUSED, char *buf ATTRIBUTE_UNUSED,
		     ...
		     int note_type ATTRIBUTE_UNUSED, ...)
{
  switch (note_type)
	...
	return elfcore_write_note (abfd, buf, bufsiz, "CORE", note_type, data,
				   sizeof (data));
      }
	...
	char *data, *ret;
	va_list ap;
	long pid, pr_reg_length = 0;
	int cursig;
	const void *greg;

	switch (elf_elfheader (abfd)->e_flags & ELF_KVX_CORE_MASK)
	  {
	  case ELF_KVX_CORE_KV3_1:
	    pr_reg_length = KVX_KV3_1_PR_LENGTH;
	    break;
	  case ELF_KVX_CORE_KV3_2:
	    pr_reg_length = KVX_KV3_2_PR_LENGTH;
	    break;
	  default:
	    return NULL;
	  }

	data = bfd_zmalloc (PRSTATUS_OFFSET_PR_REG + pr_reg_length);
	if (!data)
	  return NULL;
	...
	memcpy (data + PRSTATUS_OFFSET_PR_REG, greg, pr_reg_length);
	va_end (ap);
	ret = elfcore_write_note (abfd, buf, bufsiz, "CORE", note_type, data,
				  PRSTATUS_OFFSET_PR_REG + pr_reg_length);
	free (data);
	return ret;
      }
...
#define elf_backend_write_core_note		\
  kvx_write_core_note
