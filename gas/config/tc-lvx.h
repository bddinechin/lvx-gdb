/* tc-lvx.h -- Header file for tc-lvx.c
...
#define TARGET_ARCH bfd_arch_lvx

#define LVX_RA_REGNO (67)
#define LVX_SP_REGNO (12)

#define O_pseudo_fixup O_md1


/* Parsing related declarations.  */
...
int insn_cond_len (const char *line);
...
extern const struct lvx_register *lvx_registers;
/* Default lvx_modifiers array.  */
extern const char ***lvx_modifiers;
/* Default lvx_regfiles array and its size */
extern const int *lvx_regfiles;
extern int lvx_regfiles_size;
/* Default values used if no assume directive is given.  */
extern const struct lvx_core_info *lvx_core_info;

struct lvx_as_options {
...
extern struct lvx_as_options lvx_options;

struct lvx_as_params {
  /* The target's ABI.  */
  int abi;
  /* The target's OS/ABI.  */
  int osabi;
  ...
  int core_set;
  /* Guard to check if LVX_ABI has been set.  */
  int abi_set;
  /* Guard to check if LVX_OSABI has been set.  */
  int osabi_set;
...
extern struct lvx_stcall lvx_stcall_info;
...
struct lvx_as_env {
  ...
  struct lvx_as_options opts;
  /* Record the parameters of the target architecture.  */
  struct lvx_as_params params;
...
extern struct lvx_as_env env;


/* Hooks configuration.  */

extern const char * lvx_target_format (void);
#undef TARGET_FORMAT
#define TARGET_FORMAT lvx_target_format ()
...
extern void lvx_end (void);
#undef md_finish
#define md_finish lvx_end
...
#define tc_frob_label(sym) lvx_frob_label(sym)
#define tc_check_label(sym) lvx_check_label(sym)
extern void lvx_frob_label (struct symbol *);
extern void lvx_check_label (struct symbol *);


/* GAS listings (enabled by `-a').  */

#define LISTING_HEADER "LVX GAS LISTING"
#define LISTING_LHS_CONT_LINES 100


#define md_start_line_hook lvx_md_start_line_hook
extern void lvx_md_start_line_hook (void);
#define md_emit_single_noop lvx_emit_single_noop()
extern void lvx_emit_single_noop (void);
...
extern int lvx_force_reloc (struct fix *);
#undef TC_FORCE_RELOCATION
#define TC_FORCE_RELOCATION(fixP) lvx_force_reloc(fixP)
...
/* Controls the resolution of fixup expressions involving the difference of two
   symbols.  */
extern int lvx_force_reloc_sub_same (struct fix *, segT);
#define TC_FORCE_RELOCATION_SUB_SAME(FIX, SEC)                                 \
  (! SEG_NORMAL (S_GET_SEGMENT((FIX)->fx_addsy))                               \
   || lvx_force_reloc_sub_same(FIX, SEC))
...
extern int lvx_validate_sub_fix (struct fix *fix);
#define TC_VALIDATE_FIX_SUB(FIX, SEG)                                          \
  (((FIX)->fx_r_type == BFD_RELOC_32 || (FIX)->fx_r_type == BFD_RELOC_16)      \
   && lvx_validate_sub_fix((FIX)))

/* Generate a fix for a data allocation pseudo-op*/
#define TC_CONS_FIX_NEW(FRAG,OFF,LEN,EXP,RELOC) lvx_cons_fix_new(FRAG,OFF,LEN,EXP,RELOC)
extern void lvx_cons_fix_new (fragS *f, int where, int nbytes,
...
extern void lvx_handle_align (fragS *);
#undef HANDLE_ALIGN
#define HANDLE_ALIGN lvx_handle_align
...
extern void lvx_cfi_frame_initial_instructions (void);
#undef tc_cfi_frame_initial_instructions
#define tc_cfi_frame_initial_instructions lvx_cfi_frame_initial_instructions

extern int lvx_regname_to_dw2regnum (const char *regname);
#undef tc_regname_to_dw2regnum
#define tc_regname_to_dw2regnum lvx_regname_to_dw2regnum

/* All LVX instructions are multiples of 32 bits.  */
#define DWARF2_LINE_MIN_INSN_LENGTH 1
#define DWARF2_DEFAULT_RETURN_COLUMN (LVX_RA_REGNO)
