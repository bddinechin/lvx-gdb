...
has_relocation_of_size (const struct lvx_reloc **relocs)
{
   ...
   * Example:
   * - call accepts only a pcrel27s2 -> allow pcrel reloc here
   * - cb accepts only a pcrel17s2 -> allow pcrel reloc here
...
int
insn_cond_len (const char *line)
{
  const char *str = line;
  const char *real_insn_start = NULL;

  if (strncmp ("guard", str, sizeof ("guard") - 1)
      && strncmp ("blend", str, sizeof ("blend") - 1))
    return 0;

  /* Advance up to the end of the blend instruction.  */
  while (*str && *str != '?')
    str += 1;
  str += 1;
  real_insn_start = str;
  ...
  int n = longest_match (str, strlen (str), env.insns);
  /* Check if the token under the cursor is a valid mnemonic. */
  if (!n)
    return 0;
  ...
  /* Check that the end-of-the-line was not reached.  If so, this means that the
     valid mnemonic found previously was an immediate behind a symbol which
     shares the same name as a valid instruction.  */
  if (!*str || *str == '\n')
    return 0;

  return real_insn_start - line;
}
...
static int
lev (const char *src, const char *dst) {
#define min3(a,b,c) ( (a) < (b) ? ((a) < (c) ? (a) : (c)) : ((b) < (c) ? (b) : (c)) )
#define dist_at(i, j) (dist[((i) % 2) * len_dst + (j)])
  int i, j;
  int len_src, len_dst;
  int res = 0;
  int *dist = NULL;
  ...
  if (!len_src) return len_dst;
  if (!len_dst) return len_src;

  if (!(dist = calloc (2 * (len_dst + 1), sizeof (*dist))))
    as_fatal ("allocation failed.");
  ...
  for (i = 0; i < len_src ; ++i)
    {
      ...
      for (j = 0 ; j < len_dst ; ++j)
        {
          dist_at(i + 1, j + 1) = min3(
              dist_at(i, j + 1) + 1,
              dist_at(i + 1, j) + 1,
              dist_at(i, j) + (src[i] != dst[j])
          );
        }
    }

  res = dist_at (len_src, len_dst);
  free (dist);
  return res;
#undef dist_at
#undef min3
...
/* Rewrite with as_bad. */
static void
rule_expect_error (char *tok, int rule_id, char *buf, int bufsz __attribute__((unused)))
{
  int i = 0;
  int pos = 0;
  int comma = 0;
  ...
  if (rule_id != 0)
    {
      pos += sprintf (buf + pos, "Instruction `%s' ", tok);
      ...
      while (rules[i].steering != -1)
	{
	  if ((env.opts.allow_all_sfr || rules[i].steering != env.sys_reg)
	      && rules[i].steering != -3)
	    {
	      const char *candidate = TOKEN_NAME (rules[i].steering);
	      pos += sprintf (buf + pos, "%s%s", comma ? ", " : "", candidate);
	      comma = 1;
	    }
	  i += 1;
	}
	pos += sprintf (buf + pos, "] at the position marked by ^.");
    }
  else
    {
      pos += sprintf (buf + pos, "Did you meant to use one of [");
      int size = 0;
      while (env.token_classes->insn_classes[size].class_id != -1)
	size += 1;

      int *distances = NULL;
      if (!(distances = malloc (size * sizeof (*distances))))
	as_fatal ("allocation error");
      ...
      while (rules[i].steering != -1)
	{
	  if ((env.opts.allow_all_sfr || rules[i].steering != env.sys_reg)
	    ...
	    {
	      const char *candidate = TOKEN_NAME (rules[i].steering);
	      distances[i] = i << ((CHAR_BIT * sizeof (int)) >> 1);
	      distances[i] += lev (tok, candidate);
	    }
	  i += 1;
	}
	...
	return _a - _b;
      ...
      qsort (distances, size, sizeof (int), dist_cmp);

      for (int j = 0; j < 5; ++j)
	{
	  int r = distances[j] >> ((CHAR_BIT * sizeof (int)) >> 1);
	  pos += sprintf (buf + pos, "%s%s", comma ? ", " : "", TOKEN_NAME (rules[r].steering));
	  comma = 1;
	}
...
parse (struct token_s tok)
{
  ...
  if (!tok_list)
    {
      tok.begin = tok.end = error_char;
      read_token (&tok);
      if (tok.begin == 0 && tok.end == 0)
	{
	  /* read_token failed, we use the following heuristic to find
	     the wrong mnemonic.  */
	  for (int i = 0; tok.insn[i]
	                  && tok.insn[i] != '$' && tok.insn[i] != ' '; ++i)
	    tok.end += 1;
	}
      if (error_code != -1)
	{
	  ...
	  const char *msg = "Unexpected token when parsing `%s'.";
	    for (int i = 0; i < (int) (strlen (msg) + error_char + 1 - 5) ; ++i)
	      buf[i] = ' ';
	    for (int i = 0; i < tok.end - tok.begin; ++i)
	      buf[strlen (msg) + error_char + i + 1 - 5] = '^';
	  as_bad (msg, tok.insn);
	  if (env.opts.diagnostics)
	    {
	      as_bad ("%s", buf);
	      char err_buf[10000] = { 0 };
	      int insn_len = strlen (tok.insn);
	      char *insn = calloc (insn_len + 1, sizeof (char));
	      tok.begin = tok.end = 0;
	      read_token (&tok);
	      if (tok.begin == 0 && tok.end == 0)
		{
	      ...
	      for (int i = 0; i < tok.end - tok.begin; ++i)
		insn[i] = tok.insn[tok.begin + i];
	      rule_expect_error (insn, error_code, err_buf, 10000);
	      free (insn);
	      as_bad ("%s", err_buf);
	    }
	}
