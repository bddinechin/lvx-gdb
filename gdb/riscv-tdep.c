			     ...
			     struct regcache *regcache, int flen)
{
  ...
  memcpy (tmp, data, len);
  regcache->cooked_write (regnum + rv_regnum_ofs, tmp);
}
		       ...
		       CORE_ADDR struct_addr)
{
  ...
  if (return_method == return_method_struct)
    {
      ...
      regcache->cooked_write (RISCV_A0_REGNUM + rv_regnum_ofs, buf);
    }
  ...
  regcache_cooked_write_unsigned (regcache, RISCV_RA_REGNUM + rv_regnum_ofs,
				  bp_addr);
  ...
  regcache_cooked_write_unsigned (regcache, RISCV_SP_REGNUM + rv_regnum_ofs,
				  sp);
	...
	case riscv_arg_info::location::in_reg:
	  {
		...
		regcache->cooked_read_part (regnum + rv_regnum_ofs, 0,
